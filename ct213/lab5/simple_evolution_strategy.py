...
class SimpleEvolutionStrategy:
    ...
    def tell(self, fitnesses):
        ...
        previous_mean = self.m

        m = np.zeros(np.size(self.m))
        C = np.zeros((np.size(self.m), np.size(self.m)))

        for i in range(np.size(best_samples, 0)):
            m += best_samples[i]
            coef_2d = (best_samples[i] - previous_mean)[np.newaxis]
            transpose = coef_2d.transpose()
            C += transpose * coef_2d  # np convention
        self.m = m / mu
        self.C = C / mu
        self.samples = np.random.multivariate_normal(self.m, self.C, population_size)
