...
def fit_hill_climbing():
    ...
    def neighbors(theta):
        ...
        """
        neighbors_list = [theta + np.array([delta, 0]), theta + np.array([delta/np.sqrt(2), delta/np.sqrt(2)]),
                          theta + np.array([0, delta]), theta + np.array([-delta/np.sqrt(2), delta/np.sqrt(2)]),
                          theta + np.array([-delta, 0]), theta + np.array([-delta/np.sqrt(2), -delta/np.sqrt(2)]),
                          theta + np.array([0, -delta]), theta + np.array(*[delta/np.sqrt(2), -delta/np.sqrt(2)])]
...
def fit_simulated_annealing():
    ...
    def random_neighbor(theta):
        ...
        neighbor = theta + np.array([delta * cos(angle), delta * sin(angle)])
        return neighbor
    ...
    def schedule(i):
        ...
        :rtype: float.
        """
        temperature = temperature0/(1 + beta*i*i)
        return temperature
