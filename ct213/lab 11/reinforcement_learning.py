...
def epsilon_greedy_action(q, state, epsilon):
    ...
    if random_number < epsilon:
        actions = q[state, :]
        return np.random.randint(0, np.size(actions))
    else:
...
class Sarsa(RLAlgorithm):
    ...
    def get_greedy_action(self, state):
        ...
        :rtype: int.
        """
        return RLAlgorithm.get_exploratory_action(self, state)

    def learn(self, state, action, reward, next_state, next_action):
        current_s_a = self.q[state, action]
        next_s_a = self.q[next_state, next_action]
        self.q[state, action] = current_s_a + self.alpha * (reward + self.gamma * next_s_a - current_s_a)
...
class QLearning(RLAlgorithm):
    ...
    def learn(self, state, action, reward, next_state, next_action):
        current_s_a = self.q[state, action]
        next_s_a = np.max(self.q[next_state, :])
        self.q[state, action] = current_s_a + self.alpha * (reward + self.gamma * next_s_a - current_s_a)
