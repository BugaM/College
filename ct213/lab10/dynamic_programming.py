    ...
    while iteration < num_iterations:
        ...
        for i in range(dimensions[0]):
            for j in range(dimensions[1]):
                ...
                for action in range(NUM_ACTIONS):
                    r = grid_world.reward(current_state, action)
                    new_value[i, j] += policy[i, j, action] * r
                    for next_state in grid_world.get_valid_sucessors((i, j), action):
                        transition_prob = grid_world.transition_probability(current_state, action, next_state)
                        new_value[i, j] += grid_world.gamma * policy[i, j, action] * transition_prob * value[next_state]
        if np.max(abs(new_value - value)) < epsilon:
            break
        value = new_value
        iteration += 1
    return value
...
def value_iteration(grid_world, initial_value, num_iterations=10000, epsilon=1.0e-5):
    ...
    iteration = 0
    while iteration < num_iterations:
        max_value = -inf * np.ones(dimensions)
        new_value = 0
        for i in range(dimensions[0]):
            for j in range(dimensions[1]):
                current_state = (i, j)
                for action in range(NUM_ACTIONS):
                    ...
                    for next_state in grid_world.get_valid_sucessors((i, j), action):
                        transition_prob = grid_world.transition_probability(current_state, action, next_state)
                        new_value += grid_world.gamma * transition_prob * value[next_state]
                    if new_value > max_value[i, j]:
                        max_value[i, j] = new_value
                    new_value = 0
        if np.max(abs(max_value - value)) < epsilon:
            break
        value = max_value
        iteration += 1
    return max_value
