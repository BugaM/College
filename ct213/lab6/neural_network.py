...
class NeuralNetwork:
    ...
    def compute_gradient_back_propagation(self, inputs, expected_outputs):
        ...
        weights_gradient[2] = (delta[2] @ a[1].T)
        biases_gradient[2] = np.sum(delta[2], axis=1, keepdims=True)
        delta[1] = (self.weights[2].T @ delta[2]) * (sigmoid_derivative(z[1]))
        weights_gradient[1] = (delta[1] @ inputs.T)
        biases_gradient[1] = np.sum(delta[1], axis=1, keepdims=True)
    ...
    def back_propagation(self, inputs, expected_outputs):
        ...
        for i in range(1, 3):
            ...
            self.weights[i] = self.weights[i] - self.alpha * weights_gradient[i]/m
            self.biases[i] = self.biases[i] - self.alpha * biases_gradient[i]/m
